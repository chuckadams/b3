          ...
          <tr>
            <td colspan="2" align="center"><!-- show next/previous X posts -->
              <form name="previousXposts" method="get"><?php
                  if ($previousXstart > 0) {
                      ?>
                    <input type="hidden" name="poststart" value="<?= $previousXstart ?>"/>
                    <input type="hidden" name="postend" value="<?= $previousXend ?>"/>
                    <input type="submit" name="submitprevious" class="search" value="< <?= $posts ?>" /><?php
                  }
                  ?>
              </form>
            </td>
            <td>
              <form name="nextXposts" method="get">
                <input type="hidden" name="poststart" value="<?= $nextXstart ?>"/>
                <input type="hidden" name="postend" value="<?= $nextXend ?>"/>
                <input type="submit" name="submitnext" class="search" value="<?= $posts ?> >"/></form>
            </td>
          </tr>
        </table>
    </tr>
    <tr>
      <td valign="top" width="200"><!-- show X first/last posts -->
        <form name="showXfirstlastposts" method="get">
          <input type="text" name="posts" value="<?= $posts ?>" style="width:40px;" /?>
      ...
      <td valign="top"><!-- show post X to post X -->
        <form name="showXfirstlastposts" method="get">
          <input type="text" name="poststart" value="<?= $poststart ?>" style="width:40px;" /?>&nbsp;to&nbsp;<input type="text"
                                                                                                                    name="postend"
                                                                                                                    value="<?= $postend ?>"
                                                                                                                    style="width:40px;" /?>&nbsp;<select name="order">
            <option
                value="DESC" <?php
            if ($i === "DESC") {
                echo " selected";
          ...
          <select name="cat" style="width:140px;">
            <option value="all">All Categories</option>
              ...
              while ($row = mysqli_fetch_object($result)) {
                  ...
                  if ((string)$row->cat_ID === (string)$postdata["Category"]) {
                      echo " selected";
                  }
  ...
  <table cellspacing="0" cellpadding="5" border="0" width="100%">
      ...
      while ($row = mysqli_fetch_object($result)) {
        ...
        <tr>
          <td>
            <p>
              <b><?php the_time('Y/m/d @ H:i:s'); ?></b> [ <a href="b2edit.php?p=<?= $id ?>&c=1"><?php comments_number() ?></a>
                <?php
                if (($user_level > $authordata[13]) || ($user_login === $authordata[1])) {
                    echo " - <a href=\"b2edit.php?action=edit&post=" . $postdata["ID"];
                    if ($m) {
                        echo "&m=$m";
                    }
                    echo "\">Edit</a>";
                    echo " - <a href=\"b2edit.php?action=delete&post="
                        . $postdata["ID"]
              ...
              <br/>
              <span style="color: #999999; "><b><a href="<?php permalink_single($blogfilename); ?>" title="permalink"><?php the_title() ?></a></b> by <b><?php the_author() ?>
                  (<a href="javascript:profile(<?php the_author_ID() ?>)"><?php the_author_nickname() ?></a>)</b>, in <b><?php the_category() ?></b></span><br/>
              ...
              if (($withcomments) || ($c)) {
                  ...
                  if ($resultc) {
                    ...
                    <a name="comments"></a>
                    <p><b><span style="color: #ff3300; ">::</span> comments</b></p>
                      ...
                      if ($comment_error) {
                          echo "<p><span style=\"color: red; \">Error: please fill the required fields (name & comment)</span></p>";
                      }
                      ?>

                    <p><b><span style="color: #ff3300; ">::</span> leave a comment</b></p>
                    ...
                    <form action="b2comments.post.php" method="post">
                      ...
                      <input type="text" name="author" class="textarea" value="<?= $user_nickname ?>" size="20" tabindex="1"/><br/>
                      <input type="text" name="email" class="textarea" value="<?= $user_email ?>" size="20" tabindex="2"/><br/>
                      <input type="text" name="url" class="textarea" value="<?= $user_url ?>" size="20" tabindex="3"/><br/>
            ...
            <td colspan="2" align="center"><!-- show next/previous X posts -->
              <form name="previousXposts" method="get"><?php
                  if ($previousXstart > -1) {
                    ...
                    <input type="hidden" name="poststart" value="<?= $previousXstart ?>"/>
                    <input type="hidden" name="postend" value="<?= $previousXend ?>"/>
                    <input type="submit" name="submitprevious" class="search" value="< Previous <?= $posts ?>" /><?php
                  }
                  ?></form>
            </td>
            <td>
              <form name="nextXposts" method="get">
                <input type="hidden" name="poststart" value="<?= $nextXstart ?>"/>
                <input type="hidden" name="postend" value="<?= $nextXend ?>"/>
                <input type="submit" name="submitnext" class="search" value="Next <?= $posts ?> >"/></form>
            </td>
          </tr>
        </table>
    </tr>
