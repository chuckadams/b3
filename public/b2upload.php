...
<head>
  ...
  <style type="text/css">
    ...
    .uploadform {
      background-color: white;
    <?php if ($is_winIE) { ?> filter: alpha(opacity:100);
    <?php } ?> border-width: 1px;
      border-color: #333333;
      border-style: solid;
      padding: 2px;
      margin: 1px;
...
if (!empty($_POST)) { //$img1_name != "") {
    ...
    if (!str_contains(strtolower($fileupload_allowedtypes), strtolower($imgtype))) {
...
if (str_contains($img1_type, 'image/')) {
