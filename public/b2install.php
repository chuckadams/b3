...
$q = mysqli_query($connexion,$query) or db_doh("doh, can't create the table \"$tableposts\" in the database.", $query, mysqli_error($connexion));
...
$q = mysqli_query($connexion,$query) or db_doh("doh, can't insert a first post in the table \"$tableposts\" in the database.", $query, mysqli_error($connexion));

echo "posts: OK<br />";

// $query = "DROP TABLE IF EXISTS $tablecategories";
// $q = mysqli_query($connexion,$query) or db_doh("doh, can't drop the table \"$tablecategories\" in the database.");

$query="CREATE TABLE $tablecategories (cat_ID int(4) NOT NULL auto_increment, cat_name TINYTEXT not null , KEY (cat_ID))";
$q = mysqli_query($connexion,$query) or db_doh("doh, can't create the table \"$tablecategories\" in the database.", $query, mysqli_error($connexion));

$query = "INSERT INTO $tablecategories (cat_ID, cat_name) VALUES ('0', 'General')";
$q = mysqli_query($connexion,$query) or db_doh("doh, can't set the default category in the table \"$tablecategories\" in the database.", $query, mysqli_error($connexion));

$query = "UPDATE $tableposts SET post_category=\"1\"";
$result = mysqli_query($connexion,$query) or db_doh("Oops, can't set the default category on $tableposts.", $query, mysqli_error($connexion));

echo "b2categories: OK<br />";

// $query = "DROP TABLE IF EXISTS $tablecomments";
// $q = mysqli_query($connexion,$query) or db_doh("doh, can't drop the table \"$tablecomments\" in the database.");
...
// $q = mysqli_query($connexion,$query) or db_doh("doh, can't drop the table \"$tablesettings\" in the database.");
...
$q = mysqli_query($connexion,$query) or db_doh("doh, can't create the table \"$tablesettings\" in the database.", $query, mysqli_error($connexion));
...
$q = mysqli_query($connexion,$query) or db_doh("doh, can't set the default settings in the table \"$tablesettings\" in the database.", $query, mysqli_error($connexion));

echo "settings: OK<br />";

// $query = "DROP TABLE IF EXISTS $tableusers";
// $q = mysqli_query($connexion,$query) or db_doh("doh, can't drop the table \"$tableusers\" in the database.");
